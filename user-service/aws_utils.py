...
_PREFIX = os.getenv("APP_SECRETS_PREFIX", "arogya/dev/")
...
def get_database_url(db_name: str) -> str:
    ...
    creds = get_secret(f"{_PREFIX}db-credentials")
...
def get_rsa_private_key() -> str:
    ...
    return get_secret(f"{_PREFIX}jwt-private-key")
...
def get_rsa_public_key() -> str:
    ...
    if env_key:
        return env_key.replace("\\n", "\n")
    return get_secret(f"{_PREFIX}jwt-public-key")
