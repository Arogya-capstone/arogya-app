...
async def lifespan(app: FastAPI):
    ...
    # Bucket
    bucket = os.getenv("S3_BUCKET_NAME", "arogya-documents")
    try:
...
async def confirm_upload(
    ...
    # Trigger rag-worker via SQS — this is what actually populates pgvector
    # Without this message, the document is stored in S3 but never embedded/indexed
    try:
        sqs_client = boto3.client("sqs", region_name=os.getenv("AWS_DEFAULT_REGION", "us-east-1"))
        queue_url   = os.getenv("SQS_RAG_QUEUE_URL")
        if queue_url:
            ...
            logger.info(f"Sent document {doc.id} to RAG queue for processing")
        else:
            logger.warning("SQS_RAG_QUEUE_URL not set — skipping RAG indexing")
    except Exception as e:
        # Non-fatal: document is uploaded, RAG indexing can be retried later
        logger.error(f"Failed to enqueue document {doc.id} for RAG processing: {e}")
