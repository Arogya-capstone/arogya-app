...
from sqlalchemy import Column, Text, DateTime, Boolean, Index, text
...
async def call_huggingface(message: str, timeout: float = 10.0) -> str:
    hf_token = get_secret(f"{os.getenv('APP_SECRETS_PREFIX', 'arogya/dev/')}hf-api-key")
...
async def call_groq(message: str, timeout: float = 5.0) -> str:
    groq_token = get_secret(f"{os.getenv('APP_SECRETS_PREFIX', 'arogya/dev/')}groq-api-key")
